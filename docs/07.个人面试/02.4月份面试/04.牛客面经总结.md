...
- [ ] 一致性hash，虚拟节点作用，为什么一致性hash是一个环。
  ...
  先参考[一致性hash环原理 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/146011745)

- [ ] 10g乱序文件找中位数，思路即可。你的思路[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)的瓶颈在哪，是内存[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)，还是读写磁盘文件？

  [海量数据中寻找中位数 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/75397875)

- [ ] linux怎么管理内存？扯虚拟内存格式，malloc，缺页中断，页表，换出[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)（FIFO、LRU、CLOCK）等等。

  主要参考这个： [浅谈Linux内存管理 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/67059173)
...
- [ ] mysql有哪几种日志？一般可能答个redo undolog可能就完事了，但真的有那么简单吗？完全可以继续说有中继日志慢查询日志错误日志查询日志等，然后继续扯下去（中继日志扯到主从同步，慢查询日志扯到sql调优），让面试官觉得就算你是背的也是背透了，你的知识面是能够发散的。
...
- [ ] [客户端](https://www.nowcoder.com/jump/super-jump/word?word=客户端)和服务端建立socket连接的过程，相关的方法名

- [ ] 写多生产者多消费者模型

- [ ]  IO 密集型和 CPU 密集型如何分配线程优先级

- [ ] 有对[项目](https://www.nowcoder.com/jump/super-jump/word?word=项目)和系统做性能测试吗？(benchmark 和 pprodf)
...
- [ ] map的[源码](https://www.nowcoder.com/jump/super-jump/word?word=源码)最重要

- [ ] GMP中，M的数量怎么控制，P呢？

- [ ] hash表，jdk1.8后的hashtable很有说头，数组+[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)+[红黑树](https://www.nowcoder.com/jump/super-jump/word?word=红黑树)+rehash都能展开来讲

- [ ] [哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)，拉链法，开放地址法，大量冲突怎么优化
...
- [ ] tcp四次挥手，time_wait，2MSL

- [ ] docker的网络模式，docker如何处理容器网络访问？

- [ ] traceroute实现原理

- [ ] 介绍一下ICMP协议

- [ ] 假设没有time_wait,一个新的连接起来了，旧的报文会到达到这个连接里吗

- [ ] go是如何实现map的

- [ ] [哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)的时间复杂度、空间复杂度
...
- [ ] [链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)怎么检查环，怎么算环的长度
...
- [x] 64匹马，8个赛道，找出最快的4匹马，只能看马在赛道之间的快慢，手上没有表来记时间，问最少比赛多少次可以找到最快4匹马 参考:
...
- [ ] 讲讲HTTP与HTTPS的区别；TLS是哪一层；HTTPS的连接流程是怎样；
...
- [ ] TOPK[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)，找出一个未[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)数组中第K大的数；
...
- [ ] 三次握手详细过程？SYN攻击底层原理是什么？（要答到内核的半连接队列）
...
- [ ] 64匹马，8个赛道，最少比多少次能找到最快的4匹。（这问题一定要看）
...
- [ ] goroutine 泄露 说一下，并指出了简历上“goruntine”的拼写问题(可恶的输入法。)
